...
（注意，输出结果中请不要返回参数中的节点引用，否则判题程序会直接返回空）。 下图是一个含有5个结点的复杂链表。图中实线箭头表示$next$指针，虚线箭头表示$random$指针。为简单起见，指向$null$的指针没有画出。

![img](../post_images/JZ35_describe_1.png)
...
![img](../post_images/JZ35_describe_2.png)
